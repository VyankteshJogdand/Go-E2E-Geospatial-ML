...
- `--is_forecasting_task`: If True, input imagery is fetched from before the observation date (forecasting mode). If False, imagery is co-temporal with the observation. (default: True)
- `--num_steps`: Number of temporal steps (imagery acquisitions) to retrieve around or before the observation date. Controls the temporal depth of the model input — higher values capture more temporal context but require more data to be available. Independent of `--is_forecasting_task`. (default: 3, minimum: 1)
