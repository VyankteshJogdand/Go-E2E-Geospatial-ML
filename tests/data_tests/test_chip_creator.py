...
@pytest.mark.parametrize(
    "data_source, temporal_tolerance, chip_counts, num_bands",
    [("HLS", "1", 2, 6), ("S2", "1", 2, 6), ("S1", "3", 2, 2)],
)
def test_chip_creator(
    ...
    dataset_csv = os.path.join(output_directory, f"{data_source.lower()}_dataset.csv")
    assert os.path.exists(dataset_csv)
    assert len(pd.read_csv(dataset_csv)) == chip_counts
